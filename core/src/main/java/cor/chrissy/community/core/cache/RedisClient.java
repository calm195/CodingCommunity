package cor.chrissy.community.core.cache;
...
public class RedisClient {
    ...
    private static final String KEY_PREFIX = "coding_";
    ...
    public static <T> Boolean hDel(String key, String field) {
        return template.execute((RedisCallback<Boolean>) connection ->
                connection.hDel(keyBytes(key), valBytes(field)) != null && connection.hDel(keyBytes(key), valBytes(field)) > 0);
    }

    public static <T> Boolean hSet(String key, String field, T ans) {
        return template.execute((RedisCallback<Boolean>) redisConnection ->
                redisConnection.hSet(keyBytes(key), valBytes(field), valBytes(ans)));
    }
    ...
    public static <T> Boolean sIsMember(String key, T value) {
        return template.execute((RedisCallback<Boolean>) connection ->
                connection.sIsMember(keyBytes(key), valBytes(value)));
    }
    ...
    public static <T> boolean sPut(String key, T val) {
        return template.execute((RedisCallback<Long>) connection ->
                connection.sAdd(keyBytes(key), valBytes(val))) > 0;
    }
